...
RSpec.describe "clusters/new", type: :view do
  ...
  it "renders form with server_type action" do
    expect(rendered).to have_selector("form[action='#{clusters_path}']")
  end
  ...
  it "renders form with cluster-form directive" do
    expect(rendered).to have_selector("form[cluster-form='0']")
  end
...
RSpec.describe "server_types/new", type: :feature, js: true do
  ...
  let!(:node_roles) { create_list(:node_role, 5) }
  ...
  context "when json response is received" do
    context "when click a plus button" do
      before { page.find("button[ng-click='addClusterPart()']").click }
    ...
    context "when click a plus and a minus button" do
      before do
        page.find("button[ng-click='addClusterPart()']").click
  ...
  context "when sends empty data" do
    ...
    it "renders form with server_types action" do
      expect(page).to have_selector("form[action='#{clusters_path}']")
    end
