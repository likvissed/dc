module OmniAuth
  module Strategies
    class OpenIdLk
      ...
      option :sign_in, 'http://dc-dev/users/sign_in'
...
=begin
      def get_server
        form = OmniAuth::Form.new(title: "Адрес сервера", url: callback_path)
        form.label_field('OpenID Identifier', options.server_param)
        form.input_field('url', options.server_param)
        form.button "Перейти"
        form.to_response
      end
=end
